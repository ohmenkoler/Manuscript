...
%	\fill[dotted] (O.center) to[out=0,in=-90] ++(\e)
	...
	% ----------- Cavity
	\fill[pattern=north east lines,pattern color=MatlabOrange] (-\L/2,\l) node[below right, color=MatlabOrange]{$T_{0}$} rectangle ++(-\e,-\l);
	\fill[pattern=north east lines,pattern color=MatlabBlue] (\L/2,\l) node[below left, color=MatlabBlue]{$T_{\infty}$} rectangle ++(\e,-\l);
	\draw[ultra thick] (-\L/2,0) rectangle ++(\L,\l);
	\draw (-\L/4,0) node(O)[above, 
...
%	\end{scope} 
		...
		\draw[<->,dashed] (\L/2+4*\e,0) -- ++(0,\l) node[midway,fill=white]{$L_c$};	
