...
%		\draw[->,dashed,gray,very thick] (B.north east) -- ++(0,\l/3) arc (180:0:\l/4) -- ++(0,-\l/3) arc (0:-90:\l/4);
		...
		% ----------- Cavity
		\fill[pattern=north east lines,pattern color=MatlabOrange] (-\L/2,-\e) rectangle ++(\L,\e) node[above left, color=MatlabOrange]{$T_{c}$};
		\fill[pattern=north east lines,pattern color=MatlabBlue] (-\L/2,\l+\e) rectangle ++(\L,-\e) node[below left, color=MatlabBlue]{$T_{f}$};
		\draw[ultra thick] (-\L/2,0) rectangle ++(\L,\l);% node(O2)[below left]{$T_{\infty}$};
		\draw (-\L/2,5*\l/6) node[right, 
