...
%		
		...
		\draw[->,dashed,gray,very thick] (.1*\L/2,.5*\l) arc [start angle=180, end angle=-90, x radius=.2*\L, y radius=.4*\l];
		\draw[->,dashed,gray,very thick] (-.1*\L/2,.5*\l) arc [start angle=0, end angle=270, x radius=.2*\L, y radius=.4*\l];
...
%		\draw[->,dashed,gray,very thick] (B.north east) -- ++(0,\l/3) arc (180:0:\l/4) -- ++(0,-\l/3) arc (0:-90:\l/4);
		...
		% ----------- Cavity
		\fill[pattern=north east lines,pattern color=MatlabOrange] (-\L/2,-\e) rectangle ++(\L,\e) node[above left, color=MatlabOrange]{$T_{0}$};
		\fill[pattern=north east lines,pattern color=MatlabBlue] (-\L/2,\l+\e) rectangle ++(\L,-\e) node[below left, color=MatlabBlue]{$T_{\infty}$};
		\draw[ultra thick] (-\L/2,0) rectangle ++(\L,\l);% node(O2)[below left]{$T_{\infty}$};
		\draw (-\L/2,5*\l/6) node[below left, rotate=90, 
%		fill=white, fill opacity=.7, text opacity=1, rounded corners
		]{Adiab.}; 
...
%		\draw (0,\ylab) node[]{\textbf{\footnotesize(b)}};
		...
		\draw[<->,dashed, white] (\L/2+3*\e,0) -- ++(0,\l) node[midway,fill=white]{$L_c$};	
