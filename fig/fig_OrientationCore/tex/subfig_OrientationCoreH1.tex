...
%		\end{tabular}};
	...
	\begin{pgfonlayer}{background}
		\draw[->, very thick] (NewO.center) -- ++(22.5:\R) node [above] {$\mathbf e_{y,0}$};
		\draw[->, very thick] (NewO.center) -- ++(90:\R) node [left] {$\mathbf e_{z,0}$} node[right=1cm, draw, rounded corners, anchor=south]{\Large `\texttt{H1}'};
		\draw[->, very thick] (NewO.center) -- ++(-45:\R) node [right] {$\mathbf e_{x,0}$};
  	\end{pgfonlayer}
\end{tikzpicture}