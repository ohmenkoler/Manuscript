...
%\draw{[white](\L/2,0) -- ++(0,\R);
	...
	\begin{scope}[yslant=-1]
		...
		\node at (\decalage-\HX,1.4*\R/2) (NewO) {};
...
%		\draw (n10.north east) node [above right, fill=white, fill opacity=0, text opacity=1]{\textcolor{MatlabBlue}{\textbf{Froid}}};
	...
	\begin{pgfonlayer}{background}
		...
		\draw[->, very thick] (NewO.center) -- ++(90:\R) node [left] {$\mathbf e_{z,0}$} node[right=1cm, draw, rounded corners, anchor=south]{\Large `\texttt{H2}'};
