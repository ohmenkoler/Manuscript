...
%    
	...
	\def\decalage{\R/2-\L/2};
		...
		\draw(\R/2,-2.9) node[rotate=-90]{\includegraphics[width=.8\textwidth]{../fig/fig_OrientationCore/tex/TACOT.png}};

		\fill[top color=MatlabOrange, bottom color=MatlabBlue, draw=black] (0,0) rectangle ++(\R,\L);
		...
		\draw (n40.south east) node [right, fill=white, fill opacity=.7, text opacity=1]{\textcolor{MatlabOrange}{\textbf{Ambiant}}};
		\draw (n00.north east) node[right, fill=white, fill opacity=.7, text opacity=1]{\textcolor{MatlabBlue}{\textbf{Froid}}};
