...
% --------------------------------------------------- Dimensions
\draw (\CHXnorth) node [above right] {\begin{tabular}{ll} Côté \\  froid\end{tabular}};
\draw (\AHXnorth) node [above left] {\begin{tabular}{rr} Côté \\ ambiant \end{tabular}  };

\draw (0,-\Ryreg) node[below left] {0};
\draw (\Lreg,-\Ryreg) node{|} node[below left] {$L_{\sf reg}$};
\draw (\CHXnorth) -- ++(-.5cm,0) node[left] {$D_{\sf reg}$};

%%% Q_TA
\draw[arw,->,draw=MatlabBlue] ({.33*\Lreg},.15*\Ryreg) -- ++(.33*\Lreg,0) node[midway, above] {$Q_{TA}$} node(MidQTA)[midway]{};
%
...
%%% Q_cond
\draw[arw,->,draw=MatlabOrange] ({.67*\Lreg},-.15*\Ryreg)  -- ++(-.33*\Lreg,0) node[midway, below] {$Q_{\sf cond}^{(x)}$} node(MidCondx1)[midway]{};
%\draw[arw,->,draw=MatlabOrange] ({.75*\Lreg},.20*\Ryreg)  -- ++(-.33*\Lreg,0) node(MidCondx2)[midway]{} node[left] {$Q_{\sf cond}^{(x)}$};

\draw[arw,->,draw=MatlabYellow] ({.25*\Lreg},.3*\Ryreg)  -- ++(0,.33*\Lreg) node[midway, left] {$Q_{\sf cond}^{(r)}$} node(MidCondr1)[midway]{};
\draw[arw,->,draw=MatlabYellow] ({.75*\Lreg},-.3*\Ryreg)  -- ++(0,-.33*\Lreg) node[midway, right] {$Q_{\sf cond}^{(r)}$} node(MidCondr2)[midway]{};
...
%%% Q_lat
\draw[->,line width=1mm,decorate,decoration={snake,amplitude=.4mm,segment length=2mm,post length=2mm},red] (.5*\Lreg,1.1*\Ryreg) -- ++(0,1cm) node [above, text=black] {$\left.h_r\right|_{r=D_{\sf reg}}\theta$};
\draw[->,line width=1mm,decorate,decoration={snake,amplitude=.4mm,segment length=2mm,post length=2mm},red] (.5*\Lreg,-1.1*\Ryreg) -- ++(0,-1cm) node [below, text=black] {$\left.h_r\right|_{r=0}\theta$};
...
\draw[->,line width=1mm,decorate,decoration={snake,amplitude=.4mm,segment length=2mm,post length=2mm},red] (-2*\Rxreg,0) -- ++(-1cm,0) node [midway, above, text=black] {$\left.h_x\right|_{x=0}\theta$};
\draw[->,line width=1mm,decorate,decoration={snake,amplitude=.4mm,segment length=2mm,post length=2mm},red] (\Lreg+2*\Rxreg,0) -- ++(1cm,0) node [midway, above, text=black] {$\left.h_x\right|_{x=L_{\sf reg}}\theta$};
