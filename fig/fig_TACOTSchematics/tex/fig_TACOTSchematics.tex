\begin{tikzpicture}[scale=.5]
	...
	\node[anchor=east, inner sep=0] (imagecropped) at (image.west) {\includegraphics[angle=0,origin=c,width=.35\textwidth]{../fig/fig_TACOTSchematics/TACOT_Cropped_Feedback.png}};
...
%		\draw [white] (.41,.35) node{+};
	...
	\draw(imagecropped.south) node[below]{\hypertarget{subfig:SchemaGeneralTACOT}{\small \textbf{(a)}}};		
	\draw(SA2.south -| image.south) node{\small \textbf{(b)}};
	
%	\begin{scope}[x={(imagecropped.south east)},y={(imagecropped.north west)}, xshift=]
%	\draw[help lines, xstep=.1, ystep=.1] (0,0) grid (1,1);
%	
%	\draw (1,0.5) node{\textbullet};
%	
%%	\shade[left color=white, right color=orange] (.1,.51) rectangle (.5,.5);
%%	\shade[left color=orange, right color=white] (.5,.51) rectangle (.9,.5);
%	
%	\end{scope}
\end{tikzpicture}