\begin{tikzpicture}
   ...
   \draw[dashed] (\xone,\height) node[above] {$p_1^{(e)},~u_1^{(e)}$} -- ++(0,-1.3*\height) node (x1) [below]{$x_1$};
    \draw[dashed] (\xtwo,\height) node[above]{$p_1^{(s)},~u_1^{(s)}$} -- ++(0,-1.3*\height)node (x2) [below]{$x_2$};
...
%    
    ...
    \draw[->] (1.5*\length,3*\height/4) -- ++(1cm,0) node(e1)[midway]{} node[midway,above]{$u_1^{(e)}$};
    ...
    \draw[->] (e2.north) -- (e1.south) node[midway, left]{$p_1^{(e)}$};
    \draw[->] (s2.north) -- (s1.south) node[midway, right]{$p_1^{(s)}$};
