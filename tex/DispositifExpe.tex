...
\paragraph*{Alimentation électrique des sources} L'alimentation électrique de la source acoustique principale est mesurée au moyen d'une sonde différentielle pour la tension \echaf{et le courant ?}. Pour la source acoustique secondaire, un multimètre et une pince de courant se charge de présenter sa consommation électrique. En parallèle, les tensions des deux sources sont affichées sur un oscilloscope pour s'assurer du déphasage aux bornes des sources.
...
Tous les capteurs situés à l'intérieur de la machine sous haute pression statique sont reliées à l'extérieur via des traversées étanches \echaf{Spectite, modèle}, dimensionnées pour \qty{550}{\bar}.
...
Pour l'étude de l'influence de la gravité sur la distribution de température, le réfrigérateur doit pouvoir être orienté dans toutes les orientations utiles. Pour ce faire, il est suspendu par des palans grâce aux fixations situées à ses extrémité  et au milieu dans le sens de sa longueur. La figure~\ref{fig:TACOTSuspendu_Frigo} présente le réfrigérateur accroché à ses extrémités, et la figure~\ref{fig:TACOTSuspendu_Palans} les trois palans qui le soutiennent. Les deux palans de couleur grise, initialement présents pour régler l'inclinaison de la pompe à chaleur par rapport à l'axe horizontal, et le troisième de couleur bleue pour ajouter une direction de rotation autour de l'axe de symétrie. Celui-ci permet en outre de plus aisément passer d'une orientation à l'autre. 
