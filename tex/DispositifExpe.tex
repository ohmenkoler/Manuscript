...
%%    \externalremake
%    \input{../fig/TOC/TOC_DispositifExpe}
%%    \caption{•}
%%    \label{fig:•}
%\end{figure}
%\vfill
...
Ces trois éléments sont ensuite insérés dans un tube cylindrique qui se fixe sur le bâti de la machine pour maintenir l'espace nécessaire à la boucle de rétroaction, ce que montre la figure~\ref{fig:PhotoNoyau}.
...
%		\externalremake
	...
	\end{subfigure}	    
    \caption{Noyau thermoacoustique assemblé. \subref{fig:PhotoNoyau} photographie de l'assemblage, \subref{fig:SchemaNoyau} schéma des zones violette et jaune de la figure~\ref{fig:SchemaGeneralTACOT}. Les dimensions sont données en \unit{\centi\meter}.}
...
%	\mathbf K_p = \echaf{v_{\sf ref}\frac{\nu \Delta x}{\Delta P}},
	\echaf{\mathbf q ou \mathbf u}_{\sf ref} = -\frac{\mathbf K_p}{\echaf{\mu\,ou\,\nu}} (\nabla p - \rho_0 \mathbf g),
	\label{eq:DefPermeabilite_Wikipedia}
\end{equation}
où $\mathbf q_{\sf ref}$ est le débit d'écoulement \echaf{ordre de grandeur, moyen, de référence} d'un fluide qui s'écoule dans le milieu poreux et causé par un gradient de pression $\nicefrac{\Delta P}{\Delta x}$ \cite{nield_convection_2013}. La perméabilité ne dépend par ailleurs que du milieu, et il existe dans la littérature des formulations de la perméabilité ne prenant en compte que la géométrie interne \cite{dullien_porous_1992}. La formulation retenue s'écrit

\begin{equation}
	K_p = \echaf{\frac{4 r_h^2 \Phi}{8}},
	\label{eq:DefPermeabilite_LISN}
\end{equation}
et donne des résultats satisfaisants pour le developpement des modèles \cite{hireche_experimental_2020, baltean_gravity_2025}.\bigskip
...
\begin{table}[!ht]
    ...
    \begin{tabular}{l@{\hspace{1cm}}ll}
        ...
        Perméabilité & \qty{2.68e-10}{\meter\squared} \\
        \hline
    \end{tabular}
\end{table}
...
%		\externalremake
		...
		\label{fig:CHXschema}
	\end{subfigure}	    
    \caption{Schémas des échangeurs. \subref{fig:AHXschema} échangeur ambiant, où les trait pointillés jaune montrent le circuit d'eau, et \subref{fig:CHXschema} échangeur froid sur lequel l'emplacement des cartouches chauffantes est indiqué par les rectangles. \echaf{mettre version vierge du CHX}}
    \label{fig:HXschema}
\end{figure}

\subparagraph{\'Echangeur ambiant} 
Le flux de chaleur apporté au côté ambiant par effet thermoacoustique doit être extrait pour empêcher l'échauffement de la machine, puisque de l'énergie mécanique est continuellement ajoutée au système par les sources acoustiques. Pour ce faire, la solution retenue pour garder une température ambiante de ce côté du régénérateur est de faire circuler de l'eau dans un circuit hydraulique à l'intérieur de l'échangeur fabriqué en cuivre. L'architecture proposée est représentée sur la figure~\ref{fig:AHXschema}, et comporte cinq canaux centraux entourés d'ailettes, qui divisent le flux d'eau apporté et évacué par deux conduits périphériques. 

Le modèle \textsc{DeltaEC} ne donne pas les paramètres tels que l'épaisseur des ailettes et la distance entre elle, où les dimensions des tubes où circule l'eau. Une analyse basée sur la différence moyenne de température logarithmique quasi-statique est réalisée au moment de la conception de l'échangeur pour obtenir ces paramètres, ainsi que pour contrebalancer le manque de littérature sur les échangeurs de chaleurs immergés dans les fluides oscillants \cite{ramadan_design_2021}. \echaf{mettre les paramètre ATE}
...
\begin{figure}[!ht]
    ...
    \caption{Emplacement des thermocouples choisis pour l'étude thermique dans et autour du noyau thermoacoustique. Zoom sur les encadrés violet et jaune de la figure~\ref{fig:SchemaGeneralTACOT}. \echaf{ajouter cadre violet ?}}
...
%    	\externalremake
	...
	\end{subfigure}  	
 	\caption{Orientations choisies pour les expériences avec le réfrigérateur thermoacoustique, avec les positions des thermocouples et leurs numéro. Pour chaque cas, la gravité est orientée vers le bas de la page, soit $\mathbf g=-g\mathbf e_{z,0}$ suivant le repère défini sur la figure~\ref{fig:BancMesureComplet}. Les orientations sont \subref{fig:OrientationCore_H1}  `\texttt{H1}', \subref{fig:OrientationCore_H2} `\texttt{H2}',  \subref{fig:OrientationCore_V1} `\texttt{V1}', et  \subref{fig:OrientationCore_V2} `\texttt{V2}'.}%\textcolor{red}{CHX et AHX OK ou éch. froid et éch. chaud ? + $\psi_i$ dans la caption ou la figure ?}}
...
La troisième est choisie à un \textit{drive ratio} intermédiaire où $DR=\qty{2}{\percent}$, qui correspond à l'amplitude dite \og faible \fg{} dans les travaux de caractérisation du \textsc{Tacot} \cite{ramadan_design_2021}. \echaf{ajouter de la physique, pourquoi on choisit ces amplitudes là ? Donner l'impression qu'on fait les choses pour une bonne raison, pas comme un automate} \smallskip
