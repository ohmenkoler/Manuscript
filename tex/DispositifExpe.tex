...
Toutefois, l'empillement de tissus crée un milieu multicouche qui n'est certainement pas isotrope, et la définition de la perméabilité dans l'équation de Darcy considère un écoulement unidirectionnel, stationnaire et suffisamment lent \cite{dullien_porous_1992}. \echaf{ vraiment utile de rentrer dans les détails d'un modèle multi couche de la permeabilité ? }\cite{tan_new_2024}\medskip
...
\echaf{ajouter figure avec TC 13 14 et 15 pour montrer que peut importe ce qui se passe dans noyau l'ext reste à Ta -> pour l'instant pas trouvé de données qui sont en accord clair avec ça, on laisse ?}
...
La puissance thermique $\dot Q_f$ est apporté par effet Joule au côté froid du noyau selon la relation
