...
%    \caption{Différentes orientations du c\oe{}ur thermoacoustique avec les positions des thermocouples et leurs numéro. Pour chaque cas, la gravité est orientée vers le bas. Les orientations correspondent aux angles {\bf (a)} $\theta_v=\ang{0}$ et $\theta_h=\ang{0}$ pour l'orientation `\texttt{H1}', {\bf (b)} $\theta_v=\ang{0}$ et $\theta_h=\ang{+90}$ pour l'orientation `\texttt{H2}', {\bf (c)} $\theta_v=\ang{-90}$ pour l'orientation `\texttt{V1}', et {\bf (d)} $\theta_v=\ang{+90}$ pour l'orientation `\texttt{V2}'.\textcolor{red}{CHX et AHX OK ou éch. froid et éch. chaud ? + $\theta_i$ dans la caption ou la figure ?}}
...
\begin{figure}[!ht]
	...
	\end{subfigure}   
    \caption{Différentes orientations du c\oe{}ur thermoacoustique avec les positions des thermocouples et leurs numéro. Pour chaque cas, la gravité est orientée vers le bas. Les orientations correspondent aux angles \subref{fig:OrientationCore_H1} $\theta_v=\ang{0}$ et $\theta_h=\ang{0}$ pour l'orientation `\texttt{H1}', \subref{fig:OrientationCore_H2} $\theta_v=\ang{0}$ et $\theta_h=\ang{+90}$ pour l'orientation `\texttt{H2}', \subref{fig:OrientationCore_V1} $\theta_v=\ang{-90}$ pour l'orientation `\texttt{V1}', et \subref{fig:OrientationCore_V2} $\theta_v=\ang{+90}$ pour l'orientation `\texttt{V2}'.\textcolor{red}{CHX et AHX OK ou éch. froid et éch. chaud ? + $\theta_i$ dans la caption ou la figure ?}}
    \label{fig:OrientationCore} %
\end{figure}
...
Ensuite, la deuxième orientation est représentée sur la figure~\ref{fig:OrientationCore_H2}. Dans ce cas, référérencé en tant que `\texttt{H2}', le réfrigérateur est toujours à l'horizontale ($\theta_v=\qty{0}{\degree}$), mais pivoté autour de son axe pour placer les thermocouples sur un plan horizontal auquel la gravité est orthogonale ($\theta_h=\qty{90}{\degree}$).
\smallskip
...
\subsection{Régime transitoire sans convection naturelle}
...
Les flux thermiques sont pour la plupart différents dans le cas d'un réfrigérateur contenant un régénérateur à la place d'un \textit{stack}, et sont définis de l'équation \eqref{eq:FluxTA_Lotton_Regen} à \eqref{eq:FluxLat_Lotton}. 
...
%\begin{equation}
%Q_{\sf cond}^{(x)} = -k_x\partial_x\tau~\mathrm{\mathbf e_x},
%\label{eq:FluxCondx_Lotton}
%\end{equation}
%avec... 
%
%\begin{equation}
%Q_{\sf cond}^{(r)} = -k_r\partial_r\tau~\mathrm{\mathbf e_r},
%\label{eq:FluxCondr_Lotton}
%\end{equation}
%
%\begin{equation}
%Q_{\sf cond}^{(\alpha)} = -k_\alpha\partial_\alpha\tau~\mathrm{\mathbf e_\alpha},
