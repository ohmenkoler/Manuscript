...
Le régénérateur utilisé dans le présent travail est composé de \echaf{combien -> } disques de tissus métalliques (Gantois, modèle : 102045) empilés dans un \textcolor{purple}{enveloppe nommée canister~(photo 2.4b)} cylindrique en acier inoxydable de diamètre intérieur $D_{\sf reg}=\qty{148}{\mm}$ et de longueur $L_{\sf reg}=\qty{39}{\mm}$. Le nombre de disques de tissus pour atteindre une porosité de $\Phi=\qty{68}{\percent}$ a été déterminé en partant de la relation
...
\paragraph{Alimentation électrique des sources acoustiques} L'alimentation électrique de la source acoustique principale est mesurée au moyen d'une sonde différentielle pour la tension \echaf{et le courant ?}. Pour la source acoustique secondaire, un multimètre  et une pince de courant (Sefram, SP 280) se chargent de mesurer sa consommation électrique. En parallèle, les tensions aux bornes des deux sources sont affichées sur un oscilloscope pour s'assurer de leur déphasage.
...
\echaf{Toutes les mesures réalisées auparavant ont permis de développer un protocole fiabiliser les résultats qu'on présente après... Le protocole présenté ci-après }

\subsubsection{\'Etat initial}

Tout d'abord et pour toutes les expériences, l'état initial de toutes les grandeurs est acquis sur une minute et sauvegardé sous un label `\texttt{init}' à chaque début de journée de campagne. Cela permet de garder en mémoire toutes les conditions expérimentales initiales dont les valeurs peuvent potentiellement influer sur le comportement du réfrigérateur, comme par exemple la température ambiante ou la pression statique. En effet, le mélange de gaz contient \qty{65}{\percent} d'argon, et \qty{35}{\percent} d'hélium très volatile, d'autant plus que la pression statiques est très élevée. Au cours du temps, une partie du gaz fuit et les proportions du mélange changent. La pression du mélange gazeux au sein de la machine est mesurée dans le temps et est affichée sur la figure~\ref{fig:TenuePressionStatique}. \echaf{on y voit que la durée avant vidange est blabla}\textcolor{purple}{Quel impact sur les mesures qui se suivent}.\smallskip
...
%	\externalremake
	...
	\caption{Example de mesure de température de gaz après mise en marche du réfrigérateur, extrait de la campagne `\texttt{Acou}' dans l'orientation `\texttt{V2}' pour un \textit{drive ratio} $DR=\qty{3.5}{\percent}$.}
...
Dans chacun des types d'expérience (`\texttt{water}', `\texttt{acou}' ou `\texttt{Qc*W}'), les durées d'acquisitions sont supérieures à \qty{1}{\hour}, car le régime transitoire peut parfois présenter des comportements surprenants. C'est le cas par exemple, sur la figure~\ref{fig:Repetabilite_V1} qui représente l'évolution temporelle des températures des thermocouples 7 et 8 pour deux expériences identiques. En l'occurrence, le réfrigérateur est placé en positions `\texttt{V1}', à forte amplitude acoustique. Il est possible de remarquer d'une part que les résultats obtenus sont très différents les uns des autres, et surtout d'autre part que le régime transitoire n'est pas atteint.
