...
Pour déterminer la valeur de ce débit $u_1$ dans le cas du noyau utilisé dans le \textsc{Tacot}, un modèle réduit comprenant uniquement le régénérateur et ses échangeurs est modélisé dans le logiciel \textsc{DeltaEC}. \echaf{ajouter schéma deltaec}. La figure~\ref{fig:DeltaEC_u_opt} est obtenue en exécutant le modèle pour une amplitude $|u_1|$ variant entre les valeurs \qtylist{3e-4;4e-2}{\cubic\meter\per\second}, et pour un déphasage $\angle u_1$ de \ang{-30} à \ang{30}.
...
\begin{figure}[!ht]
	...
	\label{fig:SchemaNoyauTA_DeltaEC_u_opt}
\end{figure}
