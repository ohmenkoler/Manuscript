...
Le modèle Comsol simule 4 phénomènes. Premièrement, un flux de chaleur est retiré à la paroi en $x=0$, afin de représenter le refroidissement homogène sur la paroi par effet thermoacoustique. Ensuite, la conduction dans le métal pour $x>0$ est modélisée en utilisant deux valeur de conductivité thermique, pour l'enceinte d'une part et pour le régénérateur d'autre part. Il est à noter que dans ce modèle qui reste assez grossier, le matériau qui compose le régénérateur est supposé non poreux, afin de répondre à une hypothèse portant sur la présence ou l'absence de convection naturelle à l'intérieur de celui-ci. Pour ce faire, deux valeurs de conductivité thermiques sont considérées pour l'ensemble empilement de tissus métalliques et enceinte. 


\subsection{Modèles physiques utilisés}
\echaf{horizontal} La géométrie présentée dans la figure~\ref{fig:Comsol_ModeleSimplifie_Geometrie} est incluse dans la physique \texttt{Heat transfer in Solids and Fluids}. Les domaines correspondant au régénérateur et à son enceinte sont marqués comme solides, et le domaine représentant la cavité d'adaptation d'impédance comme fluide. Cette physique permet un couplage entre les trois domaines, en particulier entre les domaines fluides et solides. Cependant, la gravité et la convection naturelle ne sont pas pris en compte dans le modèle fluide, et il est nécessaire d'ajouter deux modèles à ce domaine concerné. Les physiques \texttt{laminar flow} et \texttt{Heat transfer in fluids}, couplé par la multiphysique \texttt{Nonisothermal flow} permettent cette modélisation plus avancée.

\echaf{vertical Rayleigh Bénard requiert de virer la multiphysique, et d'ajouter une source volumique -> utiliser nombre de rayleigh + cf ppt sur le site de comsol}
...
\begin{figure}[!ht]
    ...
    \caption{Géométrie équivalente pour établir la condition de température à l'interface en $r=-e_{\sf enc}$ entre le régénérateur et l'enceinte qui le contient, soit $r=R_{\sf reg}$. \echaf{ajouter conductivité}}
