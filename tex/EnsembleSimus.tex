...
\begin{subequations}
	\begin{align}
		\frac{1}{g} \partial_t \theta &= \partial_{xx}^2 \theta + \gamma \partial_{rr}^2\theta + \beta, & x \in [0,L_{\sf reg}],\ r \in [0,2R_{\sf reg}],\ t>0\ ; \label{eq:BilanThermique_ChgtVariables} \\
...
\end{subequations}
\echaf{changer les noms $g, \gamma, q_0, q_L$} avec les changements de variables suivants
...
\begin{multicols}{2}
	\begin{subequations}
		\begin{align}
			...
			\eta_r &= \frac{h_r}{k_r}, \\
			q_0 &= \frac{Q_{TA}(0)-Q_{\sf vort}(L_{\sf reg})+\epsilon Q_f}{k_x+k_{TA}(L_{\sf reg})}, \\
			q_l &= \frac{Q_{TA}(L_{\sf reg})+Q_{\sf vort}(L_{\sf reg})+\epsilon Q_a}{k_x+k_{TA}(L_{\sf reg})},
		\end{align}
		
		\begin{align}
			g(x) &= \frac{k_x + k_{TA}(x)}{\Phi \rho_0 C_p + (1-\Phi) \rho_s C_s}, \\
			\gamma &= \frac{k_r}{k_x + k_{TA}(x)}, \\
			\beta &= \frac{Q_{\sf visq}-\partial_x q_{TA}(x)}{k_x + k_{TA}(x)}.
		\end{align}
		\label{ChgtVariables}
	\end{subequations}
\end{multicols}
