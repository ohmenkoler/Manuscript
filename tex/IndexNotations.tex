...
%    	\hline
        ...
        $C_p$ & \unit{\joule\per\kelvin} & Capacité thermique isobare du fluide \\
        $C_s$ & \unit{\joule\per\kelvin} & Capacité thermique du solide poreux \\
        $C_v$ & \unit{\joule\per\kelvin} & Capacité thermique isochore du fluide \\
        ...
        $\mathbf{g}$ & \unit{\meter\per\square\second} & Accélération de la pesanteur, $\mathbf{g}=-g \mathbf e_{z,0}$ avec $g=\qty{9.81}{\meter\per\second\squared}$ \\
%        $G$ & \echaf{?/m} & Gain thermoacoustique unitaire \\
%        $\Grashof$ & --- & Nombre de Grashof \\
        $k_0$ & \unit{\per\meter} & Nombre d'onde sans perte \\
...
%        $\Peclet$ & --- & Nombre de Péclet \\
        ...
        $\mathbf{e}_r$ & --- & Vecteur unitaire radial \\
        ...
        $T_\infty$ & \unit{\kelvin} & Température initiale \\
        $u$ & \unit{\cubic\meter\per\second} & Débit acoustique \\
        $v$ & \unit{\meter\per\second} & Vitesse acoustique \\
        $\dot W_e$ & \unit{\watt} & Puissance électrique consommée par les sources acoustiques \\
        $\mathbf{e}_x$ & --- & Vecteur unitaire axial \\
%        $\mathbf e_{x0}$ & --- & \echaf{définir} \\
        $\dot Q_c$ & \unit{\watt} & Flux de chaleur extrait par l'eau circulant dans l'échangeur ambiant \\
        $\dot Q_f$ & \unit{\watt} & Flux de chaleur apporté par les résistances chauffantes dans l'échangeur froid\\
    \end{longtable}
...
\bigskip
    ...
    \begin{longtable}{p{.1\textwidth} @{} p{.1\textwidth} @{} p{.7\textwidth}}
%    	\hline
        \multicolumn{3}{c}{Lettres grecques}  \\\hline
...
%        $\psi_h$ & \unit{\degree} & Rotation autour de l'axe de symétrie du réfrigérateur \\
 		...
 		$\theta$ & \unit{\kelvin} & Différence entre la température en un point $T_0$ et la température initiale dans la machine $T_\infty$, ${\theta=T_0-T_\infty}$\\
 		$\kappa$ & \unit{\square\meter\per\second} & Diffusivité thermique \\	
 		$\lambda$ & \unit{\meter} & Longueur d'onde acoustique \\
        ...
        $\varphi_{SA1}$ & \unit{\degree} & Phase de la source acoustique principale \\
		$\varphi_{SA2}$ & \unit{\degree} & Phase de la source acoustique secondaire \\
        $\varphi_{2-1}$ & \unit{\degree} & Déphasage entre les sources acoustiques, $\varphi_{1-2} = \varphi_{SA2} - \varphi_{SA1}$\\
        $\Phi$ & \unit{\percent} & Porosité du régénérateur \\
        $\omega_1$ & \unit{\radian\per\second} & Pulsation du signal acoustique, $\omega_1=2\pi f_1$ \\\hline
    \end{longtable}
