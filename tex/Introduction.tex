...
Ce travail de thèse a été mené entre janvier 2022 et juin 2025 au sein de l'Institut PPRIME - UPR3346 de Poitiers sous la direction de Hélène Bailliet, maîtresse de conférences HDR, et de Jean-Christophe Valière, professeur des universités, et Gaëlle Poignand, ingénieure d'étude au Laboratoire d'Acoustique de l'Université du Mans - UMR6613, dans le cadre d'un projet de recherche financé par le ministère de l'enseignement supérieur et de la recherche. Dans ces deux unités, les machines thermoacoustiques constitue une parties des thématiques de recherche, et notamment l'étude de la réfrigération thermoacoustique compacte.\medskip

Ce projet de thèse fait suite au projet ANR \textsc{TACOT}~\cite{ANR_thermo-acoustic_2019, ramadan_design_2021} dans lequel un réfrigérateur thermoacoustique coaxial compact a été mis au point. La présentation détaillée de cette machine, qui peut extraire \qty{290}{\watt} à la source froide dont la température est \qty{13.3}{\degreeCelsius}, avec un coefficient de performance de \num{1.5}, est faite dans le chapitre \ref{chap:DispositifExpe} dédié au dispositif expérimental utilisé lors de ce doctorat.\medskip

Avant de commencer l'étude de la problématique de cette thèse qui porte sur les effets de la convection naturelle au sein du réfrigérateur, quelques bases de thermoacoustique issues de la théorie linéaire de Rott~\cite{rott_damped_1969, rott_thermoacoustics_1980} sont rappelées. Les limites pour le cas de cette étude des effets de convection sont exposés, et un plan de manuscrit pour comprendre leur manifestation est proposé.

\section{Bases de thermoacoustique}
\subsection{Concepts généraux}
Les machines thermoacoustiques utilisent les interactions visqueuses et thermiques entre un gaz et un matériau solide poreux ou un empilement de plaques (\textit{stack} en anglais) placé à l'intérieur lors d’un cycle de compression-détente de ce gaz. En particulier, les pression, volume et température, qui sont liées par les lois thermodynamiques, sont des grandeurs d’intérêt en thermoacoustique dont la théorie linéaire est en grande partie écrite par Rott et Swift au cours des années 1980 \cite{rott_damped_1969, rott_thermally_1973, rott_thermally_1975, rott_thermally_1976, zouzoulas_thermally_1976, rott_thermoacoustics_1980, muller_thermally_1983}
...
Dans un moteur thermoacoustique, le chauffage d'un côté du matériau poreux apporte de l'énergie à une parcelle de gaz située à proximité de ce côté du stack. Son volume augmente alors et pousse les autres parcelles de gaz attenantes à l'intérieur du stack, en direction de l'autre extrémité plus froide. Elles cèdent alors de l'énergie au solide, leur volume diminue et elles retrouvent leur position initiale, où le cycle recommence. Finalement, un travail acoustique est généré.
...
Après avoir succinctement défini le mode de fonctionnement d'une machine, il est nécessaire de présenter de type de propagation d'ondes qui a lieu dans son noyau. Il existe deux comportements asymptotiques qui dépendent de la forme du résonateur de la machine : les ondes stationnaires et les ondes progressives. Ces deux cas sont radicalement différents, car le déphasage entre la pression acoustique $p$ et la vitesse acoustique $v$ est de \ang{90} dans le premier cas, et \ang{0} dans le second.
...
\begin{subequations}
	\begin{align}
		...
		\rho T \deriv_t s &= \bnabla\cdot (k \nabla T), &\text{(conservation de l'énergie)}\label{eq:ThermoLin_NRJ}
	\end{align}
\end{subequations}
en utilisant les variables d'intérêt suivante : la pression $p$, le débit $u$, la vitesse $v$, la masse volumique $\rho$, la température $T$ et l'entropie $s$. Elles sont considérées monochromatiques et oscillant à la fréquence $f_1=\nicefrac{\omega_1}{2\pi}$ et sont écrites selon le formalisme de Rott en adoptant la convention temporelle $e^{i\omega_1 t}$~\cite{rott_damped_1969, swift_thermoacoustic_1988, swift_thermoacoustics_2017}
...
\begin{subequations}
	...
	\noindent
		\begin{align}
			...
			v(x,y,z,t) &= \RE \left[v_1(x,y,z) e^{i\omega t}\right],\label{eq:vitesse_rott}
	\end{align}
	\begin{align}
...
\end{equation}
où $\bullet$ représente indifféremment les variables citées précédemment quand la composante continue est non-nulle.\medskip

L'équation de la conservation de la masse~\eqref{eq:ThermoLin_ConservMass} peut être réécrite avec ces variables comme

\begin{equation}
	i\omega_1 v_1 = -\deriv_x p_1 + \mu\left[\partial^2_{yy} v_1 + \partial^2_{zz}\right],
\end{equation}
...
\end{equation}
où $f_\nu$ est la moyenne de $h_\nu$ sur la section. Les fonctions visqueuses sont définies en coordonnées polaires (en remplaçant les coordonnées transverses $y$ et $z$ par la coordonnée radiale $r=\sqrt{y^2+z^2}$) dans le cas de pores cylindriques par~\cite{arnott_general_1991}
...
L'équation de la conservation de l'énergie est elle aussi réécrite avec les approximations des équations~\eqref{eq:variables_rott}, ce qui donne
...
Le comportement des machines thermoacoustiques peut être prédit en faisant usage de la théorie linéaire \cite{rott_damped_1969, rott_thermoacoustics_1980, swift_thermoacoustics_2017}. Celle-ci s'appuie sur l'hypothèse d'une propagation unidimensionnelle permettant de représenter le noyau par un ensemble de matrices de transfert.
...
Comme déjà expliqué plus haut, le processus thermoacoustique se produit dans un matériau poreux placé dans le guide d'ondes des machines thermoacoustiques. La définition des termes constituant la matrice de transfert change alors, en considérant un élément court de longueur $\deriv x$ d'un empilement selon l'équation
...
Bien que la théorie linéaire de Rott, au travers notamment de logiciels de simulations spécialisés tels que \textsc{DeltaEC}, permette de prédire le comportement et les performances des machines thermoacoustiques, de nombreux effets sont négligés~\cite{DeltaEC}. Le vent acoustique \cite{so_internal_2006, bailliet_acoustic_2001, ramadan_experimental_2018}, les écoulements turbulents et la formations de tourbillons, la génération d'harmoniques supérieures et la convection naturelle sont autant d'effets non linéaires qui ne respectent pas les hypothèses retenues pour les modèles. Ce dernier effet est en trois dimensions, ce qui rend la théorie classique à une dimension insuffisant pour prendre en compte son influence sur la distribution de température. Il est par ailleurs assez peu étudié, bien qu'il a déjà été observé que le déclenchement de moteurs thermoacoustique y est très sensible. De plus, les températures le long des dimensions transverses du noyau peuvent aussi être inhomogènes \cite{ross_influence_2003, ramadan_experimental_2018, hireche_numerical_2019}. Dans le cas des pompes à chaleur et réfrigérateurs pour lesquelles les gradients de températures sont plus faibles que pour les moteurs, la littérature sur la convection naturelle est encore plus rare, ce qui motive ce projet de recherche \cite{zhang_novel_2011, babaei_investigation_2010}.
