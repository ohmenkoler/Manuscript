...
Ce projet de thèse fait suite au projet ANR \textsc{TACOT}~\cite{ANR_thermo-acoustic_2019} \echaf{dans lequel}
...
\begin{subequations}
	\begin{align}
		...
		\rho T \deriv_t s &= \bnabla\cdot (k \nabla T) + \left(\overline{\overline{\sigma}} \cdot \bnabla\right) \cdot \mathbf v, &\text{(conservation de l'énergie)}\label{eq:ThermoLin_NRJ}
	\end{align}
\end{subequations}
...
\end{equation}
ce qui, en reportant le résultat de l'équation~\eqref{eq:GazParfait_notationDifferentielle} dans l'équation de conservation de la masse~\eqref{eq:ThermoLin_ConservMass} en tenant compte du résultat de l'équation~\eqref{eq:T_1_moyenne_fkappa}, donne
\begin{equation}
...
Le comportement des machines thermoacoustiques peut assez simplement être prédit en faisant usage de la théorie linéaire \cite{rott_damped_1969, rott_thermoacoustics_1980, swift_thermoacoustics_2017}. Une des hypothèses de ces modèles concerne la propagation d'ondes unidimensionnelles au c\oe{}ur de celles-ci, et donc la possibilité de représenter le noyau par un ensemble de matrices de transfert.
...
\echaf{notation $p_s$ à changer en $p_1^{(s)}$ etc.}
